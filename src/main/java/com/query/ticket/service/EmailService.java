...
public class EmailService {
    ...
    @Async("emailTaskExecutor")
    public void sendTicketAssignedEmail(String to, String agentName, String ticketId, String title, String assignedBy) {
        send(to, "New ticket assigned to you — QueryDesk",
                buildHtml("Ticket Assigned", "You have a new assignment",
                        "Hi <strong>" + agentName + "</strong>, a ticket has been assigned to you by <strong>" + assignedBy + "</strong>.",
                        title, "View Ticket", frontendUrl + "/tickets/" + ticketId,
                        "Please update the ticket status as you progress.", "#c8f04a"));
    }

    @Async("emailTaskExecutor")
    public void sendTicketStatusChangedEmail(String to, String name, String ticketId, String title, String newStatus) {
        ...
        send(to, "Ticket status updated — QueryDesk",
                buildHtml("Status Updated", "Your ticket status changed to " + newStatus.replace("_", " "),
                        "Hi <strong>" + name + "</strong>, the status of your ticket has been updated.",
                        "Status: <strong style=\"color:" + color + "\">" + newStatus.replace("_", " ") + "</strong>",
    ...
    public void sendTicketEscalatedEmail(String to, String name, String ticketId, String title) {
        send(to, "Your ticket has been escalated — QueryDesk",
                buildHtml("Ticket Escalated", "Your ticket has been escalated",
                        "Hi <strong>" + name + "</strong>, your ticket has been escalated to a higher priority.",
                        title, "View Ticket", frontendUrl + "/tickets/" + ticketId,
    ...
    public void sendUserCreatedEmail(String to, String name, String tempPassword) {
        send(to, "Welcome to QueryDesk — Your account is ready",
                buildHtml("Welcome", "Your account has been created",
                        "Hi <strong>" + name + "</strong>, an administrator has created an account for you.",
    ...
    public void sendRoleChangedEmail(String to, String name, String newRole) {
        send(to, "Your role has been updated — QueryDesk",
                buildHtml("Role Updated", "Your account role has changed",
                        ...
                        "Your access level has changed. Please re-login if you notice any issues.", "#c8f04a"));
    }
    ...
    public void sendAccountStatusEmail(String to, String name, boolean enabled) {
        ...
        send(to, "Your account has been " + action + " — QueryDesk",
                buildHtml("Account " + (enabled ? "Enabled" : "Disabled"),
                        ...
                        "Hi <strong>" + name + "</strong>, your QueryDesk account has been <strong>" + action + "</strong>.",
                        enabled ? "You can now login and access the platform." : "Contact your administrator if you think this is a mistake.",
        ...
        return """
            ...
            <body style="margin:0;padding:0;background:#0a0a0f;font-family:'Helvetica Neue',Arial,sans-serif;">
              <table width="100%%" cellpadding="0" cellspacing="0" style="background:#0a0a0f;padding:40px 20px;">
                <tr><td align="center">
                  <table width="520" cellpadding="0" cellspacing="0" style="background:#12121f;border-radius:20px;border:1px solid #1e1e2e;overflow:hidden;">
                    <tr><td style="padding:28px 36px 20px;border-bottom:1px solid #1e1e2e;">
                      <table cellpadding="0" cellspacing="0"><tr>
                        <td style="background:%s;width:34px;height:34px;border-radius:8px;text-align:center;vertical-align:middle;">
                          <span style="font-size:17px;font-weight:900;color:#0a0a0f;">Q</span>
                        </td>
                        <td style="padding-left:10px;"><span style="font-size:15px;font-weight:700;color:#fff;">QueryDesk</span></td>
                      </tr></table>
                    </td></tr>
                    <tr><td style="padding:28px 36px;">
                      <p style="color:#8888aa;font-size:11px;text-transform:uppercase;letter-spacing:1px;margin:0 0 6px;">%s</p>
                      <h1 style="color:#fff;font-size:20px;font-weight:700;margin:0 0 14px;letter-spacing:-0.5px;">%s</h1>
                      <p style="color:#8888aa;font-size:14px;line-height:1.7;margin:0 0 20px;">%s</p>
                      <div style="background:#0a0a0f;border-radius:10px;padding:12px 16px;margin:0 0 20px;border-left:3px solid %s;">
                        <p style="color:#fff;font-size:14px;margin:0;">%s</p>
                      </div>
                      <table cellpadding="0" cellspacing="0" style="margin:0 0 20px;">
                        <tr><td style="background:%s;border-radius:10px;">
                          <a href="%s" style="display:inline-block;padding:11px 26px;color:#0a0a0f;font-size:14px;font-weight:700;text-decoration:none;">%s →</a>
                        </td></tr>
                      </table>
                      %s
                    </td></tr>
                    <tr><td style="padding:16px 36px;border-top:1px solid #1e1e2e;">
                      <p style="color:#3a3a5a;font-size:11px;margin:0;text-align:center;">© 2026 QueryDesk · Automated notification</p>
                    </td></tr>
                  </table>
                </td></tr>
              </table>
            ...
            """.formatted(accent, badge, heading, body, accent, highlight,
                ...
                footer.isBlank() ? "" : "<p style=\"color:#5a5a7a;font-size:12px;\">" + footer + "</p>");
    }
}